...
const fs = require('fs');
const input = JSON.parse(fs.readFileSync('build/contracts/SinglyLinkedList.json'));
...
async function test() {
    ...
    let singlyLinkedList = new caver.klay.Contract(input.abi, instance.contractAddress);
    let before = Date.now();
    console.log(`>>>>>>>>>> [AddUser] <<<<<<<<<< before: `+before);
    await singlyLinkedList.methods.addUser(0).send({
        from: caver.klay.accounts.wallet[0].address,
        gas: 100000,
        gasPrice: gasPrice
    }).then((result) => {
    ...
    await singlyLinkedList.methods.getUserList().call({
        from: caver.klay.accounts.wallet[0].address,
        gas: 100000,
        gasPrice: gasPrice
    }).then((result) => {
